...
const MoodGraph = ({ moodHistory }) => {
  ...
  return (
    <div className="w-full">
      {/* Graph Container */}
      <div className="relative bg-white/60 backdrop-blur-sm rounded-2xl p-8 border border-white/40">
        {/* Title */}
        <div className="flex items-center justify-between mb-16">
          <h3 className="text-xl font-semibold" style={{ color: '#1E252B' }}>Mood Trends</h3>
          <div className="flex items-center space-x-2">
            <div className="w-3 h-3 rounded-full" style={{ background: 'linear-gradient(135deg, #3C91C5 0%, #5A7D95 100%)' }}></div>
            <span className="text-sm font-medium" style={{ color: '#475569' }}>Last 7 days</span>
          </div>
        </div>
              ...
              return (
                <div key={index} className="flex flex-col items-center space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    {dayData.latestMood && (
                      <div 
                        className="absolute -top-8 left-1/2 transform -translate-x-1/2 text-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                      >
                    ...
                    {/* Tooltip */}
                    {dayData.latestMood && (
                      <div className="absolute -top-16 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs rounded-lg px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap z-10 max-w-32">
        ...
        {moodData.some(day => day.latestMood) && (
          <div className="mt-4 p-3 rounded-lg text-center" style={{ background: 'rgba(60, 145, 197, 0.05)' }}>
